...
```<align=left>text```
```<align=right>text```
```<align=justified>text</align>```
```<align=flush>text</align>```
...
### Gradient
```<gradient="PresetName">text</gradient>```
```<gradient="PresetName">text```
...
### Line Indent
```<line-indent=10>text</line-indent>```
```<line-indent=10>text```
